...
class InfoLetra:
    ...
    def __init__(self, link):
        ...
        self.__letra = self.__bs_letra\
        .find("div", "cnt-letra").find_all('p')[0].contents
    ...
    def letra(self):
        ...
        """
        letra = ''.join(map(str, self.__letra)).replace("<br/>"," \n ")\
            .replace("</br>"," \n ")\
            .replace("<br>"," \n ")
        return  letra
...
def recupera_letra(row, infos_letras):
    """
        Adiciona ao DataFrame da Wikipedia as letras obtidas.
        
          A busca fuzzy pelo titulo ajuda a econtrar diversos titulos que possuem
        diferenças sutis, porém há dificuldade no álbum cores & valores e 
        nas músicas vida loka pt 1, 2 e intro.
    ...
    for info_letra in infos_letras:
#        if info_letra.titulo() == "Vida Loka II" and \
#        row['Canção'] == "Vida Loka II":
#            pdb.set_trace()
